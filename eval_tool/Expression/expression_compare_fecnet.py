...
def compute_features(files, model, batch_size=50, dims=2048, device='cpu',
    ...
    pred_arr = np.empty((len(files), 832))
...
def calculate_id_given_paths(paths, batch_size, device, dims, num_workers=1):
    ...
    feat1,ori_lab = compute_features_wrapp(paths[0], fecnet, batch_size,
                                        dims, device, num_workers)
    feat2,swap_lab = compute_features_wrapp(paths[1], fecnet, batch_size,
                                        dims, device, num_workers)
